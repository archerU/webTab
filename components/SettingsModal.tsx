    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn p-4">
      <div className="bg-slate-900 border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
            ...
            {/* Category Management */}
            <div className="mb-8">
                <h3 className="text-sm font-medium text-white/60 uppercase tracking-wider mb-4">Categories</h3>
                    ...
                    {categories.map(category => (
                        <div key={category.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5 hover:border-white/10 transition-colors">
                            <div className="flex items-center gap-3 text-white/90">
                                ...
                                <span className="text-xs text-white/30 bg-white/5 px-2 py-0.5 rounded-full">{category.shortcuts.length} links</span>
                            </div>
                            
                            {category.id !== 'home' && (
                                <button 
                                    onClick={() => {
                                        if (confirm(`Delete "${category.title}" and all its shortcuts?`)) {
            ...
            <div>
                <h3 className="text-sm font-medium text-white/60 uppercase tracking-wider mb-4">Appearance</h3>
